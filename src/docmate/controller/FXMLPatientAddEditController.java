...
public class FXMLPatientAddEditController implements Initializable {
        ...
        new Thread(new Task<Boolean>() {
            ...
            protected Boolean call() {
                ...
                try {
                    ...
                    if (connection != null) {
                        String query = "INSERT INTO PATIENT (DoctorId, PatientRegistrationDate, PatientUserId, " +
                                "PatientName, PatientDOB, PatientSex, PatientPhone, PatientEmail, PatientAddress, " +
                                "PatientComments) VALUES (?, ?, (CONVERT(char(6), ?, 12) + RIGHT('000' + " +
                                "CAST((SELECT COUNT(*) + 1 FROM PATIENT WHERE DoctorId = ? AND " +
                                "PatientRegistrationDate = ?) AS varchar), 3)), ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        preparedStatement.setInt(4, Doctor.getInstance().getId());
                        preparedStatement.setDate(5, Date.valueOf(LocalDate.now()));

                        preparedStatement.setString(6, patientName);
                        preparedStatement.setDate(7, Date.valueOf(patientDOB));
                        preparedStatement.setString(8, patientSex);

                        if (!patientPhone.isEmpty()) {
                            preparedStatement.setString(9, patientPhone);
                        } else {
                            preparedStatement.setNull(9, Types.VARCHAR);
                        }
                        if (!patientEmail.isEmpty()) {
                            preparedStatement.setString(10, patientEmail);
                        } else {
                            preparedStatement.setNull(10, Types.VARCHAR);
                        }
                        if (!patientAddress.isEmpty()) {
                            preparedStatement.setString(11, patientAddress);
                        } else {
                            preparedStatement.setNull(11, Types.VARCHAR);
                        }
                        if (!patientComments.isEmpty()) {
                            preparedStatement.setString(12, patientComments);
                        } else {
                ...
                try {
                    ...
                    if (connection != null) {
                        String query = "UPDATE PATIENT SET PatientName = ?, PatientDOB = ?, PatientSex = ?, " +
                                "PatientPhone = ?, PatientEmail = ?, PatientAddress = ?, PatientComments = ? " +
                                "WHERE PatientId = ?";
                        ...
                        if (!patientEmail.isEmpty()) {
                            preparedStatement.setString(5, patientEmail);
                        } else {
                            preparedStatement.setNull(5, Types.VARCHAR);
                        }
                        if (!patientAddress.isEmpty()) {
                            preparedStatement.setString(6, patientAddress);
                        } else {
    ...
    private boolean validateData() {
        ...
        // Email Validation
        if (!patientEmail.isEmpty() && !patientEmail.matches("[a-zA-Z0-9+._%\\-]{1,256}" + "@" +
